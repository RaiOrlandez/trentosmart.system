...
def prune_resolved_cases(apps, schema_editor):
    ...
    # Get all resolved or dismissed incidents and complaints, ordered by created_at descending
    resolved_incidents = list(Incident.objects.filter(status__in=['resolved', 'dismissed']).values('id', 'created_at'))
    ...
    # The first 50 resolved/closed are to be kept. The rest are to be deleted.
