...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 2. Send to our backend for verification + JWT
      const res = await api.post('/auth/google-login/', { token: idToken });
      const { access, refresh } = res.data;

      // 3. Store tokens — AuthContext will pick these up on reload
      localStorage.setItem('token', access);
      if (refresh) localStorage.setItem('refresh', refresh);

      // 4. Reload to let AuthContext decode and redirect via router guards
